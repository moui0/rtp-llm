...
std::shared_ptr<TPBroadcastResult<BroadcastTpRequestPB, BroadcastTpResponsePB>>
...
bool KVCacheMemoryConnector::prepareCopyBuffers(const std::vector<LayerBlock>& gpu_layer_blocks,
    ...
    for (const auto& lb : gpu_layer_blocks) {
        ...
        if (isNullBlockIdx(gpu_block_idx) || layer_id < 0 || layer_id >= layer_num) {
            RTP_LLM_LOG_WARNING(
                "prepare copy buffers failed, invalid gpu_block_idx or layer_id, gpu_block_idx=%d, layer_id=%d, layer_num=%u",
                gpu_block_idx,
                layer_id,
                layer_num);
            return false;
...
bool KVCacheMemoryConnector::checkKVCacheResource(const std::shared_ptr<KVCacheResourceV1>& resource) const {
    ...
    if (layer_block_ids.size() != layer_num) {
        RTP_LLM_LOG_WARNING(
            "check kv cache resource failed, layer block ids size is not equal to layer num, layer block ids size: %zu, layer num: %u",
            layer_block_ids.size(),
            layer_num);
        return false;
    }
...
void KVCacheMemoryConnector::clearCache() {
    ...
    const auto                                  items = block_cache_->steal();
    std::map<size_t, std::vector<BlockIdxType>> blocks_to_free_map;
    // Iterate in reverse order to preserve relative order in cache
    for (auto it = items.rbegin(); it != items.rend(); ++it) {
        const auto block_index = it->block_index;
        const auto block_size  = it->block_size;
    ...
    for (auto& [block_size, blocks] : blocks_to_free_map) {
        auto pool = getBlockPool(block_size);
        if (!freeBlocks(pool, blocks, /*cache_free=*/true)) {
            RTP_LLM_LOG_WARNING(
                "clear cache failed, free memory blocks failed, pool: %p, block size: %zu, blocks num: %zu",
                pool.get(),
                block_size,
                blocks.size());
        }
